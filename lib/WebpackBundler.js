    ...
    const webpack_options = this.webpack_options = {
      ...
      resolve: {
        alias: {
          ...
          settings:  path.join(__dirname, "../namespaces/settings.js"  ),
        ...
        ].concat(_.chain(fs.readdirSync(path.join(this.env.config.rootDir, "node_modules")))
          .without(".bin")
      ...
      resolveLoader: {
        modulesDirectories: [
          path.join(this.env.config.rootDir, "node_modules"),
      ...
      if(typeof this.config.define === "string"){
        ...
        for(var key in this.config.define){
          ...
          if(typeof value === "string" && value.indexOf("@") === 0){
            ...
            defines[key] = JSON.stringify(resolved_value);
          }
        }        
      }
      webpack_options.plugins.push( new webpack.DefinePlugin( defines ) );
    }
